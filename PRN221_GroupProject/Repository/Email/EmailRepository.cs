...
namespace PRN221_GroupProject.Repository
{
    public class EmailRepository : IEmailRepository
    {
        ...
        public EmailRepository(Prn221GroupProjectContext context, ISenderEmail senderEmail, IUserRepository userRepository, IProductRepository productRepository, UserManager<ApplicationUser> userManager, ICouponRepository couponRepository)
        {
            _dbContext = context;
        ...
        {
            string subject = "Notice of successful order from Click&Clack";
            ...
            var coupon = _couponRepository.GetCouponById(orderHeader.CouponId ?? "");
            double TotalBefore = 0;
            double MoneyDiscountAmount = 0;

            if (coupon != null)
            {
                TotalBefore = orderHeader.TotalPrice / coupon.DiscountAmount * 100;
                MoneyDiscountAmount = TotalBefore - orderHeader.TotalPrice;
            }
            ...
            var userEmail = !string.IsNullOrEmpty(order.Email) ? order.Email : _userRepo.FindUserByIdAsync(order.CreatedBy).Result.Email;
            await _emailSend.SendEmailAsync(userEmail, subject, body, true);
        }
